...
# XXX Autocomplete using this script only on this list of networks
my %alt_complete_networks = ('MatterMost' => 1);
...
Xchat::register(
	"Mattermost Tab Completion", "1.0500", "Mattermost Tab Completion (based on alternative completion script)"
);
...
sub complete {
	...
	my $current_network = Xchat::get_info( "network" );
	return Xchat::EAT_NONE unless $current_network and exists($alt_complete_networks{$current_network});
	...
	if( ($word !~ m{^[${command_char}]})
		...
		if( $completed ) {
			...
			if( $word_start == 0 && !$completions->{skip_suffix} ) {
				...
				Xchat::command( "settext $left$prefix$completed $right" );
				$completions->{pos} = length( "$left$prefix$completed " );
			}
